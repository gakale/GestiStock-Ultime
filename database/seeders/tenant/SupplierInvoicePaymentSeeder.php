...
class SupplierInvoicePaymentSeeder extends Seeder
{
    /**
    ...
    {
        // Générer un timestamp unique pour cette exécution du seeder
        ...
        // Utiliser le premier fournisseur pour les scénarios 1 et 2
        $supplier1 = $suppliers->first();
        
        // Utiliser le deuxième fournisseur pour le scénario 3, ou le premier si un seul existe
        $supplier2 = $suppliers->count() > 1 ? $suppliers[1] : $suppliers->first();
        
        // Utiliser les produits disponibles, peu importe leur nombre
        $product1 = $products->first();
        $product2 = $products->count() > 1 ? $products[1] : $products->first();
        $product3 = $products->count() > 2 ? $products[2] : $products->first();
        ...
        $this->createScenario1($supplier1, $product1, $user, $timestamp);

        // Scénario 2: Une facture partiellement payée
        $this->createScenario2($supplier1, $product2, $user, $timestamp);

        // Scénario 3: Un paiement pour plusieurs factures
        $this->createScenario3($supplier2, $product3, $user, $timestamp);
